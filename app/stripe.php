...
/* =========================================================
   DESTROY POINTS WHEN USER BECOMES FREE

   Points belong only to users with active paid membership
   or active Scout access.

   A Stripe subscription ending must NOT destroy points if
   the person still has an active Scout profile.

   Once the user has neither paid membership nor active Scout
   access, existing point values are permanently zeroed.

   Scout activity/history remains for audit purposes.
   ========================================================= */

function llama_destroy_points_if_free(
    PDO $db,
    int $userId,
    ...
    /*
     * These Stripe-derived states still represent continuing
     * membership access.
     */
    ...
    /*
     * Scout access is independent of Stripe. A Scout whose
     * old paid subscription finally ends must keep points
     * while the Scout profile remains active.
     */
    ...
    /*
     * The user is now genuinely free.

     * Point destruction is permanent. Historical activity
     * stays in place, but its point values cannot later be
...
function llama_sync_stripe_subscription(
    ...
    /*
     * cancel_at_period_end alone does not remove points.
     *
     * Stripe still reports the subscription as active until
     * the paid period actually ends. Point destruction occurs
     * only once Stripe reports a non-member state and the
     * user also has no active Scout access.
     */

    llama_destroy_points_if_free(
        $db,
        $userId,
        $membershipStatus
    );
